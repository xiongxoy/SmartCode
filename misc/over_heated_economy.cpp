...
#define REP(i, n) for (int i=0;i<int(n);++i) // repeat n times, counter is i, ++i is better than i++
#define FOR(i, a, b) for (int i=int(a);i<int(b);++i) // repeat from a to b, excluding b, counter is i
#define DWN(i, b, a) for (int i=int(b-1);i>=int(a);--i) // count down from b to a, excluding b, counter is i
#define REP_1(i, n) for (int i=1;i<=int(n);++i) //  repeat from i, n times
#define FOR_1(i, a, b) for (int i=int(a);i<=int(b);++i) // FOR_1
...
// V for vector, S for set, M for map
...
inline DB getArea(vector<Po>& P){
    DB res = 0; REP(i, SZ(P)-1)
        res += det(P[i], P[i+1]);
    return fabs(res);
}

/* It's a great pleasure to read your code */
struct Polygen{
    vector<Po> P, C;
    int n, nn;
    ...
    void getConvex(){
        ...
        P.pop_back(), SRT(P), P.PB(P[0]); C.resize(n+1); nn = -1; REP(i, n){
            while (nn > 0 && dett(C[nn-1], C[nn], P[i]) <= 0) --nn;
            C[++nn] = P[i];
        }

        int _nn = nn; DWN(i, n, 0){
    ...
    bool legal(){
        REP(i, n){
            int j = (i + 1) % n; FOR(ii, i+2, n){
                if (ii == i || ii == j) continue; int jj = (ii + 1) % n; if (jj == i || jj == j) continue;
                if (isIntersect(Seg(P[perm[i]], P[perm[j]]), Seg(P[perm[ii]], P[perm[jj]]))) return 0;
            }
        }

        vector<Po> T; REP(i, n) T.PB(P[perm[i]]); T.PB(P[perm[0]]);
        return sgn(getArea(T), A) > 0;
    }

    void solve(){
        input(), getConvex(), A = getArea(C)/2;
        CLR(perm); REP(i, n) perm.PB(i);
        while (!legal()) next_permutation(ALL(perm));
